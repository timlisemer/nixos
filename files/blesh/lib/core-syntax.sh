...
function ble/syntax:bash/cclass/initialize {
  ...
  # _ble_syntax_bash_chars[_ble_ctx_ARGI] は以下で使われている
  #   ctx-command (色々)
    ...
    if rex='^(\$?")([^'"${_ble_syntax_bash_chars[_ble_ctx_QUOT]}"']*)("?)' && [[ $tail =~ $rex ]]; then
      ...
      if [[ ${BASH_REMATCH[3]} ]]; then
        ...
        if (((ctx==_ble_ctx_PWORD||ctx==_ble_ctx_PWORDE||ctx==_ble_ctx_PWORDR)&&aqdel!=_ble_attr_QDEL)); then
          # _ble_ctx_PWORD (パラメータ展開) でクォート除去が有効でない文脈の場合、
...
# 文脈 _ble_ctx_PATN (extglob/case-pattern)
...
# 文脈 _ble_ctx_BRAX (bracket expression)
...
## _ble_ctx_CONDI 及び _ble_ctx_RDRS の時は不活性化したブレース展開として振る舞う。
## _ble_ctx_RDRF 及び _ble_ctx_RDRD, _ble_ctx_RDRD2 の時は複数語に展開されるブレース展開はエラーなので、
...
function ble/syntax:bash/check-brace-expansion {
  ...
  # Note: _ble_ctx_VRHS 等のときは inactive なので此処には来ないので OK
...
# 文脈 _ble_ctx_BRAX (brace expansion)
...
# ${_ble_syntax_bash_chars[_ble_ctx_ARGI]} により読み取りを行っている
...
function ble/syntax:bash/check-tilde-expansion {
  ...
  if ((tilde_enabled)); then
    ...
    #   _ble_syntax_bash_chars[_ble_ctx_PWORD] ではなく
    #   _ble_syntax_bash_chars[_ble_ctx_ARGI] を修正して使う。
...
function ble/syntax:bash/check-variable-assignment {
  ...
  if ((ctx==_ble_ctx_CMDI||ctx==_ble_ctx_ARGVI||ctx==_ble_ctx_ARGEI&&${#rematch2})); then
    # 変数代入のときは ctx は先に _ble_ctx_VRHS, _ble_ctx_ARGVR に変換する
    ...
    # 変数代入以外のときは = が現れて初めて _ble_ctx_ARGQ などに変換する
    ((_ble_syntax_attr[i]=ctx,
      i+=${#rematch}))
  fi
  ...
  if [[ $rematch2 == '[' ]]; then
    ...
    if [[ $variable_assign ]]; then
      ...
      # → ble/syntax:bash/check-glob 内で nest-push "$_ble_ctx_BRAX" 'a[' し、
      # → ble/syntax:bash/ctx-bracket-expression で抜けた後で = があれば文脈値設定
    fi
  elif [[ $rematch2 == *'=' ]]; then
    if [[ $variable_assign && ${text:i} == '('* ]]; then
      # var=( var+=(
      # * nest-pop した直後は未だ _ble_ctx_VRHS, _ble_ctx_ARGVR の続きになっている。
      #   例: a=(1 2)b=1 は a='(1 2)b=1' と解釈される。
      #   従って ctx (nest-pop 時の文脈) はそのまま (_ble_ctx_VRHS, _ble_ctx_ARGVR) にする。
...
##     既定でコマンドの属性は _ble_attr_ERR にする。
##     許容するコマンドは何れも予約語なので、
##     許容された暁には自動的に _ble_attr_KEYWORD で上書きされるのでOK
##
##     予約語以外に一致する時には、
##     明示的に属性値 _ble_attr_ERR をキャンセルする必要がある。
##
  ...
  if ((ctx==_ble_ctx_CMDI)); then
    ...
    if local rex='^([ 	]*)(\([ 	]*\)?)?'; [[ ${text:i} =~ $rex && $BASH_REMATCH ]]; then
      ...
      if [[ $rematch2 == '('*')' ]]; then
        ...
        # Note: 単語の種類が _ble_ctx_CMDX0 の時はそのままにする。
  ...
  then
    ...
    if [[ $BASH_REMATCH == ';' ]]; then
      if ((ctx==_ble_ctx_FARGX2||ctx==_ble_ctx_FARGX3||ctx==_ble_ctx_CMDXD0)); then
        ...
        # Note #D0592: time ; 及び ! ; に限っては、エラーにならずに直後に _ble_ctx_CMDXE になる
...
function ble/syntax:bash/ctx-command-compound-expect {
  ...
  if ble/syntax:bash/starts-with-delimiter-or-redirect; then
    ...
    if ((ctx==_ble_ctx_FARGX2)) && [[ $tail == [$';\n']* ]]; then
      ...
      # for ((...)) の場合
      # ここで return せずに以降の _ble_ctx_CMDX1 用の処理に任せる
...
function ble/syntax:bash/ctx-command-time-expect {
  ...
  # 期待する単語でない時は _ble_ctx_CMDXT に decay
  if ((ctx==_ble_ctx_TARGX1)); then
    # Note: bash-5.1 では "--" が来てもOKなので
    #   ctx=_ble_ctx_TARGX2 として次の if で処理させる。
...
function ble/syntax:bash/ctx-redirect/check-word-begin {
  if ((wbegin<0)); then
    # ※解析の段階では _ble_ctx_RDRF/_ble_ctx_RDRD/_ble_ctx_RDRD2/_ble_ctx_RDRS の間に区別はない。
...
## 文脈値 _ble_ctx_HERE0, _ble_ctx_HERE1
##
...
function ble/syntax/completion-context/prefix:quote/.check-container-word {
  # Note: _ble_ctx_QUOTE は nest の中にあるので、一旦外側に出て単語を探す。
...
function ble/syntax/completion-context/prefix:brace {
  # (1) _ble_ctx_BRACE{1,2} 以外になるまで nest を出る
...
function ble/syntax/completion-context/.check-here {
  ...
  if [[ ${stat[0]-} && ${_ble_syntax_completion_context_check_here[stat[0]]-} ]]; then
    # Note: ここで _ble_ctx_CMDI や _ble_ctx_ARGI は処理しない。既に check-prefix で引っ
    #   かかっている筈だから。
    #
