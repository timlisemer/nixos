...

{
  ...
  systemd.user.services.downloadWindows11 = {
    ...
    Service = {
      ...
      ExecStart = "${pkgs.writeShellScript "download-windows11" ''
        if [ ! -d $HOME/.config/qemu/setup/windows11 ]; then
          ${pkgs.coreutils}/bin/mkdir -p $HOME/.config/qemu/setup/
          cd $HOME/.config/qemu/setup
  ...
  home.file.".local/share/applications/windows11-qemu.desktop".text = ''
    ...
    Exec=quickemu --vm $HOME/.config/qemu/setup/windows-11.conf
    Icon=$HOME/.local/share/icons/windows11-480.png
    Terminal=false
    Type=Application
