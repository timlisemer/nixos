...

{
  ...
  home.file.".local/share/applications/windows11-qemu.desktop".text = ''
    ...
    Exec=bash -c 'VM_FILES="$HOME/.config/qemu/windows-11"; [ ! -d "$VM_FILES" ] || [ -z "$(ls -A "$VM_FILES")" ] && mkdir -p "$VM_FILES" && cd "$VM_FILES" && quickget windows 11; quickemu --vm "$VM_FILES/windows-11.conf"'
    Icon=/home/tim/.local/share/icons/windows11-48.png
    Terminal=false
    Type=Application
    Categories=System;Virtualization;
    StartupWMClass=.qemu-system-x86_64-wrapped
  '';

  # Create a .desktop entry for the Windows 10 VM
  home.file.".local/share/applications/windows10-qemu.desktop".text = ''
    [Desktop Entry]
    Name=Windows 10
    Comment=Launch Windows 10 in QEMU
    Exec=bash -c 'VM_FILES="$HOME/.config/qemu/windows-10"; [ ! -d "$VM_FILES" ] || [ -z "$(ls -A "$VM_FILES")" ] && mkdir -p "$VM_FILES" && cd "$VM_FILES" && quickget windows 10; quickemu --vm "$VM_FILES/windows-10.conf"'
